...
**Date:** 2026-08-03
**Status:** ✅ SUCCESS — 259/259 tests passing (12 files), setup suite 104/104, 0 failures; reviewer APPROVED (after one ⚠️ CHANGES REQUESTED cycle)
**Pipeline:** implementer (bootstrap) → implementer (planning) → coder (Batch A: 7 parallel) → coder (Batch B: 3 parallel) → unit-tester (independent validation) → reviewer (⚠️ CHANGES REQUESTED) → coder (review fixes) → reviewer (✅ APPROVED) → tracker (this entry)

### Summary

Implemented an automatic vocabulary-sync workflow: the pre-commit hook now runs a new `scripts/vocab-sync.js` that scans **staged** memory-bank files for frontmatter tags and appends any genuinely-new tags to `memory-bank/.vocabulary.json` under the `topic` group (warn-not-block, idempotent, `entity_patterns`-aware, stages the vocab so it commits with the same change). The validator (`validate-memory-schema.js`) was fixed to count `entity_patterns` keys as known tags (eliminating the `npm`-style false-positive warnings that triggered this work) and to normalize tags on both sides for parity with vocab-sync. Agent instructions (`memory-schema.instructions.md` + `reviewer.agent.md`) were updated, and the feature is wired into consumer distribution via the setup tool's hook template (`constants.js`) and `sync.js`'s `scriptsToSync`. The reviewer requested changes twice in one cycle — M1 (normalization parity) and M2 (JSON-comment instruction impossible in strict JSON) plus two minors — all fixed by the coder and APPROVED on re-review. Final feature outcome: first-time tags no longer warn (chicken-and-egg eliminated), unknown tags self-heal into the vocabulary at commit time.

### Execution Steps

| Step | Agent | Status | Result |
|---|---|---|---|
| 0 | implementer (bootstrap) | ✅ | Architecture context + memory bank verified; **graphify graph regenerated** (was missing: 2879 nodes, 3140 edges, 265 communities); memory vector index synced (2963 vectors). |
| 1 | implementer (planning) | ✅ | `plan/feature-vocab-sync-1.md` — 10 tasks / 2 batches (A: 7 parallel, B: 3 parallel). Distribution mechanism mapped: scripts via npm tarball + `scriptsToSync`, hook content via setup `TEMPLATE_HOOKS`, `.opencode/**` + `.agents/**` via sync.js. Confirmation gate PASSED by user. |
| 2 | coder (Batch A — 7 parallel) | ✅ | T1 `scripts/vocab-sync.js` (212L, 8 exports + `isDirectRun` seam); T3 validator `entity_patterns` awareness; T4 setup `constants.js` hook template (+ vocab-sync line, description updated); T5 `.husky/pre-commit` line (executable bit 100755 preserved); T6 `sync.js` `scriptsToSync` + `'vocab-sync.js'`; T8 memory-schema instructions +22L "Maintaining the Vocabulary"; T9 reviewer agent hygiene bullet. All files disjoint — no clobbering. |
| 2 | coder (Batch B — 3 parallel) | ✅ | T2 `scripts/vocab-sync.test.js` — 54 tests via real temp-git-repo fixtures (chdir + resetModules + dynamic import), spawnSync CLI exit-code suite, `isDirectRun` branch in-process; vocab-sync coverage 98.88% stmts. T7 setup hooks verify — **no test changes needed** (`expectedContent()` auto-derives); setup suite 104/104. T10 `scripts/validate-memory-schema.test.js` — 15 tests; `isDirectRun` guard + `validateFile`/`main` export seam added to validator. |
| 3 | unit-tester (independent validation) | ✅ | Full suite **254/254** (12 files, 0 failures); coverage vocab-sync 98.88% stmts / 92.15% branch, validator 92.39% stmts / 92.85% branch (both ≥ 90% gate). Non-vacuity via real temp mutations (entity_patterns merge removal → TEST-01/04 fail; append disabled → 14 fail). Scratch-repo pre-commit smoke: `brand-new-tag` appended → validator clean (no unknown-tag warning) → 2nd run no-op (idempotent). |
| 4 | reviewer | ⚠️ CHANGES REQUESTED | No critical. 254/254 + 104/104 re-verified; coverage ≥ 90%; ordering/marker-idempotency/scriptsToSync/npm files/command-injection all confirmed. **M1 (major):** validator compares RAW tags while vocab-sync normalizes → `My Tag` still warns after `my-tag` appended (chicken-and-egg only fixed for normalized tags; zero tests cover this). **M2 (major):** "add a one-line comment in JSON" is impossible in strict JSON — following it literally corrupts the vocab. Minors: `buildKnownTags` char-iterates if `vocab.tags` is an array; first-write reformats blank lines (informational); `sync.test.js` fixture omits vocab-sync.js; memory files introduce unknown tags (self-healing by design). |
| 2b | coder (review fixes) | ✅ | **M1:** `validateFile()` normalizes BOTH sides (identical `normalizeTag`: lowercase/trim/underscore+space→hyphen/drop-empty) before unknown-tag filter; +3 M1 tests. **M2:** T8 step 5 + T9 bullet reworded — rationale in commit message/PR or `categories` description strings, explicit strict-JSON no-comment warning. **Minor 1:** `Array.isArray(group)` guard in `buildKnownTags` + 2 malformed-shape tests. **Minor 3:** `'vocab-sync.js'` added to sync.test.js fixture (7 entries). `.vocabulary.json` untouched. Full suite **259/259** (12 files); coverage validator 92.7/93.1, vocab-sync 98.91/92.15. |
| 3b | reviewer (final sign-off) | ✅ APPROVED | M1 verified (normalization parity at validator lines 131-146, +3 tests, empirical scratch-repo chicken-and-egg re-check: `My Tag` → `my-tag` appended → NO warning exit 0). M2 verified (grep confirms "one-line comment in the JSON" gone). Minor 1 guard before iteration; Minor 3 fixture matches real array. Independently re-ran 259/259 + 104/104. Only informational minors (stale plan wording line 221, `normalizeTag` duplication drift risk inherent to CON-01). Recommendation: APPROVE, no further changes. |
| 5 | tracker | ✅ | This entry — full pipeline record in `docs/tracker-log.md`, progress summary, learned-knowledge session, TRACKER-INDEX update, memory bank + PG knowledgebase re-index. |

### Files Produced / Modified

| File | Description |
|---|---|
| `scripts/vocab-sync.js` | **NEW** — Pre-commit hook script (216L): scans staged memory-bank `.md` files (`git diff --cached --name-only --diff-filter=ACM`, excludes `/.index/`), parses frontmatter tags (self-contained `parseFrontmatter` copy per CON-01), known-tag set = `tags.*` values + `entity_patterns` keys, normalizes candidates (lowercase kebab-case, drop empty, dedupe), appends genuinely-new tags to `topic` (sorted, idempotent), writes 2-space indent + trailing newline, `git add` in try/catch (never blocks), warn-only exit 0. 8 exports + `isDirectRun` guard. |
| `scripts/vocab-sync.test.js` | **NEW** — 54 tests covering TEST-01..14 via real temp-git-repo fixtures, spawnSync CLI exit-code suite, in-process `isDirectRun` branch. vocab-sync coverage 98.91% stmts / 92.15% branch / 100% lines. |
| `scripts/validate-memory-schema.js` | **MODIFIED** — T3: known-tags set now includes `entity_patterns` keys (kills `npm`/`vitest` false positives; `\|\| {}` guard). T10: `isDirectRun` guard + `export { validateFile, main }` testability seam. M1: `normalizeTag` on both frontmatter tags and vocab `allTags` before unknown-tag filter (normalization parity). |
| `scripts/validate-memory-schema.test.js` | **NEW** — 15 tests: entity_patterns scenarios (TEST-01..05), main() exit-code contract, validateFile error branches; +3 M1 normalization-parity tests added in Step 2b. |
| `scripts/setup/constants.js` | **MODIFIED** — `TEMPLATE_HOOKS['pre-commit'].content` now `bump-version.js` → `vocab-sync.js` → `validate-memory-schema.js` (vocab-sync BEFORE validator); description updated to 'Auto-bump version + sync vocabulary + validate memory-bank schema before commits'. |
| `.husky/pre-commit` | **MODIFIED** — `node scripts/vocab-sync.js` added between bump-version and validate-memory-schema; marker-first header + executable bit (100755) preserved. |
| `scripts/sync.js` | **MODIFIED** — `'vocab-sync.js'` added to `scriptsToSync` adjacent to `'validate-memory-schema.js'` (consumer distribution via `__scripts__/` sync). |
| `scripts/sync.test.js` | **MODIFIED** — Minor 3: `'vocab-sync.js'` added to the `scriptsToSync` fixture (7 entries, matches real array). |
| `.agents/instructions/memory-schema.instructions.md` | **MODIFIED** — +22L "Maintaining the Vocabulary" section (step 5 reworded in Step 2b: rationale in commit message/PR or `categories` descriptions; strict JSON — no comment syntax). |
| `.opencode/agents/reviewer.agent.md` | **MODIFIED** — "Memory-bank hygiene" checklist bullet added to Guidelines (checks BOTH `tags.*` + `entity_patterns`; unaddressed unknown tags → 🔵 minor findings; reworded in Step 2b). |
| `plan/feature-vocab-sync-1.md` | **NEW** — Implementation plan: 10 tasks / 2 batches, status **Completed**; §1-§12 document distribution mechanism, alternatives, risks, and post-review fixes. |
| `memory-bank/activeContext.md` | Updated during pipeline (Current Focus + Recent Changes) |
| `memory-bank/progress.md` | Updated during pipeline; tracker summary added by this entry |
| `memory-bank/tasks/_index.md` | Updated during pipeline |

### Key Decisions

- **Staged-files-only scope** — pre-commit only cares about what's being committed; scanning all memory-bank files would produce noisy vocab writes from unrelated files (ALT-02).
- **vocab-sync runs BEFORE the validator** — new tags are appended to `.vocabulary.json` before the validator's unknown-tag check, eliminating the first-commit chicken-and-egg false positive (ALT-05). Ordering is identical in both this repo's hook and the consumer `TEMPLATE_HOOKS` template.
- **`entity_patterns` keys count as known tags** — `npm`, `vitest`, `playwright`, etc. are entity names, not vocabulary gaps; the validator's false-positive warnings were the original trigger for this feature.
- **Normalization parity (M1)** — both `vocab-sync.js` and the validator apply the SAME `normalizeTag` (lowercase/trim/underscore+space→hyphen/drop-empty) so a mixed-case `My Tag` no longer warns after `my-tag` is appended. A shared module was deliberately NOT extracted (CON-01) — the duplication is accepted, with drift risk documented.
- **Default `topic` group placement** — auto-categorizing tags into `workflow`/`memory_ops`/`agent_roles` is too heuristic-prone for a hook; `topic` is the catch-all and agents recategorize manually later (ALT-03).
- **Warn-not-block contract (CON-02)** — vocab-sync exits 0 in every graceful path (no staged files → silent; missing/malformed vocab → warn + skip); `git add` is wrapped in try/catch (RISK-01); only unhandled errors exit 1.
- **Strict JSON — no comment syntax (M2)** — `.vocabulary.json` cannot contain comments; tag rationale belongs in the commit message/PR or existing `categories` description strings. The doc instruction that suggested otherwise was corrected in both instruction files.
- **Malformed-vocab guard (Minor 1)** — `Array.isArray(group)` before iterating protects against consumer-controlled malformed shapes (e.g., `vocab.tags` being an array → character iteration).
- **Consumer distribution is layered** — new script ships via npm tarball (`files` includes `scripts/`) AND `sync.js` `scriptsToSync`; hook content ships via setup `TEMPLATE_HOOKS`; `.opencode/**` and `.agents/**` ship via sync.js. Old consumers need `npm update` + re-run `npx ai-workflow-setup` (RISK-02).

### Verification Results

| Check | Result |
|---|---|
| `npx vitest run` (full suite) | ✅ 259/259 across 12 files, 0 failures (coder, unit-tester, reviewer all independently confirmed; was 185 → 254 → 259) |
| `npx vitest run scripts/setup/` | ✅ 104/104 across 6 files (hooks 16, discover 21, prepare 27, index 27, + others) |
| vocab-sync.js coverage | ✅ 98.91% stmts / 92.15% branch / 100% funcs / 100% lines (≥ 90% gate) |
| validate-memory-schema.js coverage | ✅ 92.7% stmts / 93.1% branch / 100% funcs / 92.47% lines (≥ 90% gate) |
| Mutation: remove `entity_patterns` merge | ✅ Validator TEST-01 + TEST-04 FAIL — non-vacuous |
| Mutation: disable new-tag append | ✅ 14 vocab-sync tests fail — non-vacuous |
| Pre-commit smoke (scratch temp repo) | ✅ `brand-new-tag` appended → validator clean exit 0 (no warning); `npm` recognized as entity_patterns; 2nd run idempotent no-op |
| M1 chicken-and-egg re-check (scratch repo) | ✅ `My Tag` → `my-tag` appended → validator NO warning exit 0; controls still warn |
| Consumer setup / distribution | ✅ `TEMPLATE_HOOKS` contains vocab-sync BEFORE validator; `scriptsToSync` includes `'vocab-sync.js'` (11 entries); package.json `files` covers `scripts/` + vocab |
| `node --check` on all changed JS | ✅ Clean |
| Reviewer final verdict | ✅ APPROVED — no critical/major; only informational minors (stale plan wording line 221, `normalizeTag` duplication drift risk) |

### Notes / Follow-up

- **Feature outcome:** the pre-commit hook now auto-appends unknown memory-bank frontmatter tags to `memory-bank/.vocabulary.json` (topic group) via `scripts/vocab-sync.js` (runs before the validator, warn-not-block, idempotent, entity_patterns-aware); the validator counts `entity_patterns` keys as known tags and normalizes both sides; agent instructions (memory-schema + reviewer) updated; wired to consumers via the setup tool hook template + `sync.js` scriptsToSync. Final suite: 259/259 + setup 104/104.
...
## Pipeline 10: .env Credential Remediation + TEST-05 Hardening + Commit Pipelines 7-9 Work

**Date:** 2026-08-03
**Status:** ✅ SUCCESS — full suite 15 files / 285 tests, 0 failures; 3 conventional commits (fix(security) + feat + docs); `.env` untracked with local file preserved; reviewer APPROVED (after condition #1 redaction + C1 working-tree redaction); 0 password hits repo-wide
...
Addressed all three follow-ups from the Pipeline 9 reviewer: (1) **SECURITY** — removed the tracked `.env` file (which held a live PostgreSQL `DATABASE_URL` credential) from git via `git rm --cached` + `.gitignore` while preserving the local file, and documented the non-optional manual credential rotation + history purge; (2) **TEST-05 hardening** — added a non-vacuous negative assertion to `tests/spec-kb-consolidation.test.js` that fails if a hardcoded unscoped `projectId: 'ai-workflow-template'` appears in agent specs/instructions/scripts; (3) **Commit** — the uncommitted Pipelines 7-9 work was shipped in three conventional commits (`c931cb9` fix(security), `bd5f780` feat, `efc3cf5` docs) with the secret-bearing plan file redacted first. Two security footguns were caught and closed mid-pipeline: the untracked plan file (condition #1) and uncommitted memory-bank working-tree entries that re-introduced the full password (C1). The manual steps remaining (credential rotation + `git filter-repo` history purge of `d788f68`) are user responsibilities, documented throughout.

### Execution Steps

| Step | Agent | Status | Result |
|---|---|---|---|
| 1 | implementer (planning) | ✅ | `plan/process-env-remediation-1.md` — 488L, 3 tasks / 2 batches, 12 TEST-IDs. Batch A parallel: T1 (TEST-05 negative assertion — explicit flat list of 22 files across `.opencode/.agents/scripts`, excludes `docs/plan/tests/memory-bank` where the unscoped name legitimately appears) + T2 (.env git remediation). T3 sequential (commits, must wait for T2). Key decisions: credential rotation is MANUAL (`ALTER USER` SQL documented — agent cannot reach the remote PG); two-commit strategy keeps the security fix visible (`fix(security):` + `feat:`); `git rm --cached` preserves local `.env`; history purge (`git filter-repo`) recommended but not automated; `--no-verify` for doc-only commits + manual post-commit memory-schema validation. Confirmation gate PASSED. |
| 2 | coder (Batch A — 2 parallel) | ✅ | **T1**: negative assertion added to `tests/spec-kb-consolidation.test.js` (lines 107-165, 22-file flat list; regex `/projectId\s*[:=]\s*['"]ai-workflow-template['"]/g`; scoped name does NOT match). Deviation: `learned-knowledge.instructions.md` excluded from the scan list — the historical journal legitimately documents the pattern at line 375 and would false-fail TEST-12. Targeted 12/12; full suite **15 files / 285 / 0**; **non-vacuity proven** (injected violation into `tracker.agent.md` → FAIL with offender listed → restored → PASS). **T2**: `.gitignore` +3 lines (`# Environment files` + `.env` at line 18); `git rm --cached .env` (index clean, local 1073B file preserved); `.env.example` verified safe placeholder (no change); leaked-pattern grep — password **0 hits** in code dirs; IP found only in tracked historical security docs (accepted). Note: the plan's quoted brace-glob grep had an fnmatch false-negative; re-run with separate `--include` flags caught the 1 missed ref. |
| 2 | coder (T3, sequential) | ✅ | Two conventional commits via `--no-verify` (doc-only; version bump undesired per CON-02/RISK-03): **`c931cb9`** `fix(security): stop tracking .env with live credentials` (2 files: `.gitignore` +3L, `D .env` 30L; body = manual rotation SQL + update `.env` + `git filter-repo` purge note) and **`bd5f780`** `feat: wire agents to central knowledge vector and standardize KB projectId` (29 files: protocol file, knowledgebase + learned-knowledge instructions, 10 agent specs, 3 orchestrators, 3 spec tests, `vitest.config.ts`, docs logs, memory-bank, 2 Pipelines 7-9 plan files). **Security overrides (documented deviations):** (1) `plan/process-env-remediation-1.md` left **UNTRACKED** — it then contained the real leaked password 3×; committing would re-leak into new history (overrides ALT-03); (2) REDACTED the raw password that T1/T2 siblings had written into memory-bank before staging (would have shipped the secret in Commit 2). Verified: `git ls-files .env` empty; 0 password hits in both new commit trees; post-commit `validate-memory-schema` exit 0 + `vocab-sync` idempotent; full suite 15 files / 285 / 0. |
| 3 | unit-tester (independent validation) | ✅ | Full suite **15 files / 285 / 0** exact match; spec-wiring trio **26/26** (6+8+12); TEST-05 negative assertion re-proven non-vacuous independently (temp injection → FAIL → `git checkout` restore → 12/12 pass). Git state: commit order fix→feat correct; `git ls-files .env` EMPTY; commit 1 = `.gitignore` +3L + `D .env` only; commit 2 = 29 files with only `.env.example` in tree; local `.env` preserved on disk. Security: password prefix **0 hits in both commit trees** (`git grep` exit 1); commit-2 memory-bank files ship REDACTED (`<REDACTED>`/`9sso…` truncated); IP only in historical docs (acceptable). Note: local `memory-bank/.index/memory.db` binary retained the pre-redaction value (git-ignored, NOT a repo/commit leak; optional memory_rebuild hygiene). Manual steps remain user's responsibility. |
| 4 | reviewer | ✅ APPROVED (conditions) | Security remediation verified sound: `.gitignore:16-18` has `.env`; `git ls-files .env` empty; local `.env` preserved (1073B); password **0 hits in HEAD + HEAD~1** (only pre-existing HEAD~2 `.env`); fix-commit scope exact; redaction complete (memory-bank ships `<REDACTED>`/truncated; 45/45 tags known; no doc corruption). TEST-05 negative assertion meaningful + non-vacuous (proven twice); blind-spot scan 0 matches outside exclusions. Commit hygiene: conventional style, fix→feat order, correct groupings, `--no-verify` justified. Full suite 285/285 re-run. 🟡 **Major (condition #1):** untracked `plan/process-env-remediation-1.md` held the live password (lines 52/160/172) — a **re-leak footgun for `git add -A`**; fix = redact 3 occurrences → `<REDACTED>`, verify 0 hits, commit or delete. 🔵 Minors: static flat list (RISK-06); unquoted-YAML form (theoretical); `memory_rebuild` alone insufficient — needed VACUUM (reviewer actioned: index residue purged, 0 hits). Condition #2 (manual): user must rotate credential + decide on history purge. |
| 4b | coder (condition #1) | ✅ | 3 password occurrences redacted → `<REDACTED>` (lines 52/160/172; docs/TEST-IDs/instructions preserved; kept private-LAN IP + `postgres` user — not secrets, no credential accompanies). Verified `grep -c` 0, `git grep HEAD` 0 hits, 0 untracked files, validator exit 0. Commit **`efc3cf5`** `docs: add env remediation plan (secrets redacted)` (1 file, `--no-verify` doc-only). Prior commits untouched. |
| 3b | reviewer (final sign-off) | ⚠️ CHANGES REQUESTED | Condition #1 fully resolved at HEAD (plan tracked + committed `efc3cf5`, redacted, 0 hits; commit order correct; `.env` untracked; suite 285/285). **C1 (new):** the pipeline's own uncommitted memory-bank working-tree changes (unit-tester/reviewer appended summaries) **re-introduced the FULL password verbatim in 3 files** — `activeContext.md:887`, `progress.md:2306`, `tasks/_index.md:17` (quoting validation report text). HEAD is clean (0 hits) but `git add -A` would commit the secret — same footgun class. Reviewer deliberately did NOT append memory-bank entries to avoid entrenching the leak. Fix: redact 3 lines to `<REDACTED>`/`9sso…` convention, verify 0 working-tree hits, then re-review clears to APPROVED. 🔵 Minors: local `.index/memory.db-wal` holds the value (gitignored; rebuild after C1); pre-existing history refs (user-side purge). |
| 4c | coder (C1 redaction) | ✅ | 4 literal occurrences across 3 tracked files redacted to the `9sso…`/`<REDACTED>` convention (activeContext.md:887 grep-cmd refs, progress.md:2306, tasks/_index.md:17; context preserved). Verified: `git grep -F` working tree **0 hits**; full 32-char value repo-wide **0 hits** (excl. `.env` local-by-design); HEAD clean. Memory index: `memory_update` + `memory_rebuild` purged WAL residue (0 hits in `.index/`; semantic search returns only truncated form). No commit (orchestrator decides). Self-audit: own notes written strictly truncated-form. |
| 3c | reviewer (final confirmation) | ✅ APPROVED | **C1 resolved, no further conditions.** Working-tree prefix grep 0 hits; full 32-char value 0 hits; 3 memory-bank diffs use truncated/redacted forms only (0 full-prefix occurrences, context preserved); `.index/` (memory.db/-wal/-shm) 0 hits post-rebuild; `git status` exactly 4 expected files (3 memory-bank + orchestrator log); full suite 15 files / 285 / 0; `git ls-files .env` empty; validator exit 0. 🔵 Informational: 5-char `9sso…` traceability prefix retained (cannot reconstruct secret; consistent with accepted convention). Manual user steps (rotation + history purge) remain user-executed, documented. |
| 5 | tracker | ✅ | This entry — full pipeline record, progress summary, learned-knowledge Session 2026-08-03 (tracked-secret remediation pattern), TRACKER-INDEX update, memory bank + PG knowledgebase re-index. |

### Files Produced / Modified

| File | Description |
|---|---|
| `plan/process-env-remediation-1.md` | **NEW (Step 1) → committed `efc3cf5` (Step 4b, redacted)** — 488L remediation plan (3 tasks / 2 batches, 12 TEST-IDs). The plan's own leaked-password occurrences were redacted to `<REDACTED>` before commit; kept private-LAN host + `postgres` user (not secrets). |
| `.gitignore` | **MODIFIED (T2, committed in `c931cb9`)** — +3 lines (blank + `# Environment files (may contain credentials)` + `.env` at line 18) so the credential file can never be re-added accidentally. |
| `.env` | **REMOVED FROM TRACKING (T2, `D` in `c931cb9`)** — `git rm --cached .env`; local file preserved on disk unchanged (1073B) so dotenv-based tooling (MCP server, CLI, setup) keeps working. |
| `tests/spec-kb-consolidation.test.js` | **MODIFIED (T1)** — NEW TEST-05 negative assertion (lines 107-165): 22-file flat list + regex `/projectId\s*[:=]\s*['"]ai-workflow-template['"]/g`, asserts 0 matches; `learned-knowledge.instructions.md` documented exclusion. Test file 11 → 12 tests (290L). |
| Commit `c931cb9` | **fix(security): stop tracking .env with live credentials** — `.gitignore` +3L + `D .env` 30L; body = manual rotation SQL + `.env` update + `git filter-repo` purge note. |
| Commit `bd5f780` | **feat: wire agents to central knowledge vector and standardize KB projectId** — all Pipelines 7-9 work (29 files: protocol, instructions, 10 agent specs, 3 orchestrators, 3 spec tests, vitest.config.ts, docs logs, memory-bank, 2 plan files); memory-bank entries ship REDACTED. |
| Commit `efc3cf5` | **docs: add env remediation plan (secrets redacted)** — 1 file (the remediation plan, redacted), `--no-verify` doc-only. |
| `memory-bank/activeContext.md`, `memory-bank/progress.md`, `memory-bank/tasks/_index.md` | **MODIFIED (C1 redaction, uncommitted at pipeline end)** — 4 literal full-password occurrences redacted to `<REDACTED>`/`9sso…`; plus this entry's tracker summary in `progress.md`. |

### Key Decisions

- **`git rm --cached` + `.gitignore` is the safe "stop tracking, keep working" remediation** — the file is removed from the index (stops tracking) but preserved on disk unchanged (SEC-04), so the local `.env` continues to work for all existing tooling; `.env.example` stays the safe committed template (REQ-03).
- **Two-commit strategy: `fix(security):` first, then `feat:`** — the `.env` removal is a security fix that must be VISIBLE as its own commit; combining it with the large Pipelines 7-9 feature commit would dilute the security signal in git log (ALT-02 rejected). Commit ordering (fix before feat) is also enforced by the dependency: Commit 2 must not accidentally stage `.env`.
- **Credential rotation is the non-optional minimum; history purge is user-decided** — the agent cannot rotate a remote PostgreSQL password (CON-01) nor force-push a history rewrite; rotation (`ALTER USER postgres WITH PASSWORD '<new>'` + local `.env` update) invalidates the historic exposure; `git filter-repo` (ALT-01) is documented but requires user confirmation.
- **Untracked secret-bearing files are a re-leak footgun — redact + commit or delete** — condition #1: the plan file (which contained the real password 3×) was redacted and committed (`efc3cf5`) so any future `git add -A` is safe. Deviation from the plan's ALT-03 (commit plan as-is) was justified: committing the raw secret would re-leak it into new history.
- **Redaction must cover ALL sinks, not just commit trees** — the C1 finding proved that validation-report quoting in uncommitted memory-bank working-tree files can re-introduce the full secret; the fix pattern is redact working tree → verify `git grep -F` 0 hits → rebuild the memory vector index (WAL residue purged via `memory_rebuild`, reviewer-added VACUUM).
- **`--no-verify` for doc-only commits + manual post-commit validation** — the Husky pre-commit hook auto-bumps `package.json` (undesired for doc-only changes); skipping all hooks requires re-running `node scripts/validate-memory-schema.js` + `vocab-sync` manually (RISK-08 mitigation).
- **TEST-05 negative assertion follows existing test conventions** — `readRepoFile` helper + deterministic flat file list (RISK-06 acknowledged: new agent/script files require manual list extension, which is the intended audit trigger); `learned-knowledge.instructions.md` excluded because it is a historical journal that legitimately documents the pattern.
- **The `9sso…` 5-char truncated prefix is the accepted redaction convention** — it provides traceability without reconstructability; used consistently across commits, working tree, memory-bank, and this tracker entry.

### Verification Results

| Check | Result |
|---|---|
| `npx vitest run` (full suite) | ✅ 15 files / 285 tests, 0 failures (was 284; +1 from T1 negative assertion) — confirmed by coder, unit-tester, reviewer independently |
| `npx vitest run tests/spec-kb-consolidation.test.js` | ✅ 12/12 passed |
| Spec-wiring trio (spec-knowledge-retrieval + spec-orchestrator-parity + spec-kb-consolidation) | ✅ 26/26 (6+8+12) |
| TEST-05 negative assertion non-vacuity | ✅ Proven twice (coder + unit-tester): temp-injected `projectId: 'ai-workflow-template'` stub in `tracker.agent.md` → test FAILS listing the offender → restore → PASS |
| `git ls-files .env` | ✅ Empty (`.env` not tracked after T2 / in any new commit) |
| `.env` on disk | ✅ Preserved (1073 bytes) |
| Password prefix (`9sso…`) grep | ✅ 0 hits in both commit trees (`git grep` exit 1); 0 hits in working tree after C1; 0 hits repo-wide incl. `.index/` post-rebuild |
| `git log --oneline -3` | ✅ `efc3cf5` docs ← `bd5f780` feat ← `c931cb9` fix(security) ← `a9a5b7d` (correct order) |
| Commit scopes | ✅ `c931cb9` = `.gitignore` +3L + `D .env` only; `bd5f780` = 29 files (2280+/51-); `efc3cf5` = plan file only |
| `node scripts/validate-memory-schema.js` | ✅ exit 0 (post-commit and post-redaction) |
| `node scripts/vocab-sync.js` | ✅ idempotent no-op post-commit |
| Reviewer final verdict | ✅ APPROVED — C1 resolved, no further conditions |

### Notes / Follow-up

- **MANUAL USER STEPS (non-optional)**: (1) **Rotate the credential** — the leaked password was pushed to the public GitHub remote (`d788f68`). Run `ALTER USER postgres WITH PASSWORD '<new-strong-password>'` on the PG server at `192.168.31.200` and update the local `.env` `DATABASE_URL` (never reuse the leaked value). (2) **History purge** — optionally `git filter-repo --path .env --invert-paths` + force-push to remove the credential from history (destructive; coordinate if collaborators exist).
- **The repo is public** — even with rotation, the historical credential in `d788f68` remains visible; rotation invalidates it. Do not push the new commits until the user decides on purge.
- **Tracked-secret remediation is now a closed loop**: `git rm --cached` + `.gitignore` + redact-all-sources (commit trees, working tree, memory-bank, vector index incl. WAL) + commit-split fix(security)/feat. The `9sso…` truncated prefix convention is documented in learned knowledge.
