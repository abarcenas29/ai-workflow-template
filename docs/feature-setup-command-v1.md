...
| T8 | Create `scripts/setup/sync-phase.js` — export async function `runSync(context)` per design §4.6. Spawns `scripts/sync.js` and `scripts/normalize-memory.js` as child processes (Option A from design — child process to avoid `process.exit()` in imported modules). Steps: (1) Spawn `node <packageRoot>/scripts/sync.js` with env `INIT_CWD=<consumerRoot>`, pass `--force` if context.flags.force, `--dry-run` if context.flags.dryRun. Capture stdout, stderr, exit code. (2) Spawn `node <packageRoot>/scripts/normalize-memory.js` with same env and flags. (3) Parse stdout from sync.js for summary line pattern: `[ai-workflow-template] Synced {n} files ({m} new), scaffolded {s} files, skipped {k} modified files.` (4) If sync.js exit code non-zero: warn but don't block (conforms to graceful degradation per REQ-04). (5) If normalize-memory.js exits 0: record "normalized" or "already normalized". If non-zero: warn. (6) In dry-run mode: both child processes receive `--dry-run`, making them no-op. Return `SyncResult` with `{ filesSynced: number, filesScaffolded: number, filesSkipped: number, normalized: boolean, errors: ErrorObject[] }`. | `scripts/setup/sync-phase.js` | B | T1, T2, T3 | |
...
| Task ID | Description | File(s) | Batch | Dependencies | Completed |
|---------|-------------|---------|-------|--------------|-----------|
| T11 | Add `# Managed by @abarcenas/ai-workflow-template setup` as the **first line** of `.husky/pre-commit`. The marker goes above the existing `node scripts/bump-version.js` line. The existing content follows after a blank line. | `.husky/pre-commit` | D | — | |
| T12 | Add `# Managed by @abarcenas/ai-workflow-template setup` as the **first line** of `.husky/post-merge`. The existing shebang `#!/bin/sh` moves to line 2, followed by the existing `. "$(dirname "$0")/_/husky.sh"` content. | `.husky/post-merge` | D | — | |
